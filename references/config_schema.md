...
| Field | Type | Notes |
|---|---|---|
| `template` | string | One of `bold_impact`, `clean_split`, `elegant_pills`, `torn_paper`, `photo_overlay`, `steps_timeline`, `gradient_highlights`, `night_glow`, `stamp_collage`, `magazine_split`, `luggage_tag`, `certificate_award`, `neon_edge`, `flat_pop`, `dark_chevron`, `studio_split`, `boarding_pass`, `movie_poster`, `postcard`, `chat_mockup`. See `style_guide.md` for which to pick. All 20 fit Instagram's feed-post limits with no cropping: `studio_split` renders square at 1080x1080; `neon_edge`, `flat_pop`, `dark_chevron`, and `movie_poster` render at 1080x1350 (4:5, the max); `bold_impact` is bottom-bar-pinned at up to 1080x1330; every other template crops dynamically to its actual content height, typically somewhere between 1080x750 and 1080x1350. |
